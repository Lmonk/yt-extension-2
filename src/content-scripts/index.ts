import { filterOutShorts, filterOutShortsFromRecomendations } from './default.ts'; 
...
const playListener = () => {
  ...
  chrome.runtime.sendMessage({ type: BgMessageEnum.UPDATE_TAB_DATA, payload: { isPlaying: true }});
...
const pauseListener = () => {
  ...
  chrome.runtime.sendMessage({ type: BgMessageEnum.UPDATE_TAB_DATA, payload: { isPlaying: false }});
};
...
const likedListener = (event: Event, likeElement: HTMLElement) => {
  ...
  if(currentTarget) {
    chrome.runtime.sendMessage({ type: BgMessageEnum.UPDATE_TAB_DATA, payload: { isLiked: currentTarget.ariaPressed === 'true', videoId }});
  }
};
...
const dislikedListener = (event: Event, dislikedElement: HTMLElement) => {
  ...
  if(currentTarget) {
    chrome.runtime.sendMessage({ type: BgMessageEnum.UPDATE_TAB_DATA, payload: { isDisliked: currentTarget.ariaPressed === 'true', videoId }});
  }
};
...
function addEventListenerToLikesElement() {
  const likeElement = document.querySelector('#primary-inner ytd-watch-metadata like-button-view-model button') as HTMLElement;
  const dislikeElement = document.querySelector('#primary-inner ytd-watch-metadata dislike-button-view-model button') as HTMLElement;

  if(likeElement && dislikeElement) {
    likeElement.addEventListener('click', (event) => likedListener(event, likeElement));
    dislikeElement.addEventListener('click', (event) => dislikedListener(event, dislikeElement));
  } else {
...
function addEventListenerToShortsRecomendationElement() {
  const shortsRecomendationsElem = document.querySelector('ytd-watch-next-secondary-results-renderer ytd-reel-shelf-renderer');
  console.log(shortsRecomendationsElem)
  if(shortsRecomendationsElem) {
    shortsRecomendationsElem.addEventListener('load', filterOutShortsFromRecomendations);
  } else {
...
function removeEventListenerToShortsRecomendationElement() {
  ...
  if(shortsRecomendationsElem) {
    shortsRecomendationsElem.removeEventListener('load', filterOutShortsFromRecomendations);
  }
}
...
(function() {
  ...
  window.addEventListener('load', () => {
    ...
    chrome.runtime.sendMessage({ type: BgMessageEnum.UPDATE_TAB_DATA, payload: { ...videoState } });
  });
