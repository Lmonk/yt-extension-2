...
export function sortColors(palette: Color[]): Color[] {
  const sortedPalette = palette.sort((a, b) => getLuminance(a) - getLuminance(b));
  sortedPalette.shift();
  sortedPalette.pop();
...
export function getTextAndBackgroundColors(dominantColor: Color): { textColor: Color; backgroundColor: Color } {
